  ...
  $scope.submitDecision = function(formId, entry, decision, fname, lname, email, affiliation, date_created) {
    ...
    $http.post(ifr_api + 'decision?blog=2', formData) // FIXME
      .success(function(data, status, headers, config) {
        ...
        $window.open('mailto:' + email + '?subject=' + encodeURIComponent('ITP Camp Application Status') + '&body=' + encodeURIComponent(renderMessage(ifr_decision_message[data['decision']], messageData)));
      })
      .error(function() {
        console.log('error');
      });
