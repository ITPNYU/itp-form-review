  ...
  $scope.submitDecision = function(formId, entry, decision, fname, lname, email, affiliation, date_created) {
    ...
    $http.post(ifr_api + 'decision?blog=2', formData) // FIXME
      .success(function(data, status, headers, config) {
        ...
        $window.open('mailto:' + email + '?subject=ITP%20Camp%20Application%20Status&body=' + renderMessage(ifr_decision_message[formData["decision"], {"firstName":formData["fname"], "registerLink": "https://itp.nyu.edu/camp/2014/register/?email=" + encodeURI(formData["email"])}));
      })
      .error(function() {
        console.log('error');
      });
